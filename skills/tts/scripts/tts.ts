...
generate options:
  ...
  --voice <alias|id>    Voice alias or ElevenLabs ID (default: default)
...
Environment:
  ...
  ELEVENLABS_VOICE_ID   Voice ID for the "default" alias (required)`);
  process.exit(1);
}
...
async function cmdGenerate(args: Args): Promise<void> {
  ...
  const voiceName = optionalArg(args, "voice", "default")!;
